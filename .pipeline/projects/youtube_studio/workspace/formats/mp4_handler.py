...
This module provides specific handling for MP4 video files, including
...
@dataclass
class MP4Metadata:
    """Metadata extracted from MP4 files"""
...
class MP4Handler:
    """
    Handler for MP4 video format operations.
    
    This class provides methods for validating MP4 files, extracting metadata,
    and performing format-specific operations.
    """
    
    # Supported MP4 container formats
    SUPPORTED_CONTAINERS = ['mp4', 'm4a', 'm4v', 'mov']
    
    # Common video codecs in MP4
    SUPPORTED_VIDEO_CODECS = ['h264', 'h265', 'hevc', 'vp8', 'vp9', 'av1']
    
    # Common audio codecs in MP4
    SUPPORTED_AUDIO_CODECS = ['aac', 'mp3', 'opus', 'vorbis']
    
    # Minimum valid file size for MP4 (to detect corrupted files)
    MIN_FILE_SIZE = 1024  # 1KB
    
    def __init__(self, file_path: str):
        """
        Initialize the MP4 handler with a file path.
        
        Args:
            file_path: Path to the MP4 file to handle
        """
        self.file_path = file_path
        self._metadata: Optional[MP4Metadata] = None
    ...
    def is_valid_extension(cls, file_path: str) -> bool:
        ...
        Returns:
            True if file has a valid MP4 extension
        """
    ...
    def validate_file(self) -> Tuple[bool, str]:
        ...
        if not self.is_valid_extension(self.file_path):
            return False, f"Invalid MP4 file extension: {os.path.splitext(self.file_path)[1]}"
        
        return True, "File is valid"
    
    def get_metadata(self) -> Optional[MP4Metadata]:
        """
        Extract metadata from the MP4 file.
        
        Returns:
            MP4Metadata object with file information, or None if extraction fails
        """
        if self._metadata:
    ...
    def _get_basic_metadata(self) -> Optional[MP4Metadata]:
        """
        Get basic metadata without external tools.
        
        Returns:
            Basic MP4Metadata object
        """
        file_size = os.path.getsize(self.file_path)
    ...
    def is_compatible_with_youtube(self) -> Tuple[bool, str]:
        """
        Check if the MP4 file is compatible with YouTube upload requirements.
        ...
        # Check video codec
        if metadata.video_codec not in ['h264', 'h265', 'hevc']:
            return False, f"Unsupported video codec: {metadata.video_codec}. Use h264 or h265/hevc"
        
        # Check audio codec
        if metadata.audio_codec not in ['aac', 'mp3', 'vorbis', 'opus']:
            return False, f"Unsupported audio codec: {metadata.audio_codec}. Use aac, mp3, or vorbis"
        
        # Check duration
        if metadata.duration_seconds > 12 * 60 * 60:
            return False, f"Video duration ({metadata.duration_seconds}s) exceeds YouTube limit (12 hours)"
        
        # Check file size
        if metadata.file_size_bytes > 256 * 1024 * 1024 * 1024:
            return False, f"File size ({metadata.file_size_bytes} bytes) exceeds YouTube limit (256GB)"
        
        return True, "File is compatible with YouTube"
    
    def get_thumbnail_path(self) -> str:
        """
        Get the path for a thumbnail file associated with this MP4.
        
        Returns:
            Path to the thumbnail file (filename.mp4.jpg)
        """
